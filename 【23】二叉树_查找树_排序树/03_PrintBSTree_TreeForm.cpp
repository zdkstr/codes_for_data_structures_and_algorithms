...
int BiTreeDepth(BSTree T) {  // 递归方式实现对深度的查询
	if (T == NULL)
		return 0;
...
* Y     从哪一层开始打印二叉树
*/
void Traverse(BSTree T, int depth, int right, int tap,int Y) {
	if (T == NULL) return;
	static int treeDepth = BiTreeDepth(T);  // 获取树的深度
	BSTree L=T->lchild, R=T->rchild;  // 获取左右子树
...
// 打印树形接口
void PrintBSTree_TreeForm(BSTree T) { // 注意，不同计算机能打印的层数不同
